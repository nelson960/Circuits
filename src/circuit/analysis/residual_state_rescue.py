...
RESIDUAL_POSITION_RESCUE_SCHEMA_VERSION = 1
...
def _position_limited_residual_patch(
    *,
    clean_residual: torch.Tensor,
    ablated_residual: torch.Tensor,
    batch: dict[str, Any],
    metadata: dict[str, torch.Tensor],
    position_role: str,
) -> torch.Tensor:
    if clean_residual.shape != ablated_residual.shape:
        raise RuntimeError(
            f"Clean/ablated residual shape mismatch: clean={tuple(clean_residual.shape)} "
            f"ablated={tuple(ablated_residual.shape)}"
        )
    patched = ablated_residual.clone()
    for flat_index in range(int(metadata["rows"].numel())):
        prediction_position = int(metadata["prediction_positions"][flat_index].item())
        batch_row, positions = _attention_key_positions(
            batch=batch,
            metadata=metadata,
        ...
        position_tensor = torch.tensor(positions, device=patched.device, dtype=torch.long)
        patched[batch_row, position_tensor, :] = clean_residual[batch_row, position_tensor, :]
    return patched


def _compute_position_rescue_rows_for_source(
    *,
    model: torch.nn.Module,
    checkpoint_paths_by_step: dict[int, Path],
    pairs_by_id: dict[str, dict[str, Any]],
    ...
    for step, margin_side in sorted(endpoint_keys):
        ...
        for start_index in range(0, len(pair_ids), batch_size):
            ...
            if clean_outputs.residual_streams is None or ablated_outputs.residual_streams is None:
                raise RuntimeError("Residual-position rescue requires residual streams.")
            clean_logits, clean_targets, clean_metadata = extract_answer_logits(clean_outputs.logits, batch)
            ablated_logits, ablated_targets, ablated_metadata = extract_answer_logits(ablated_outputs.logits, batch)
            _validate_single_query_batch(batch=batch, metadata=clean_metadata, label="residual-position rescue clean")
            _validate_single_query_batch(batch=batch, metadata=ablated_metadata, label="residual-position rescue ablated")
            request_specs_by_pair_id = {
                pair_id: specs_for_endpoint[pair_id]
                for pair_id in batch_pair_ids
                if pair_id in specs_for_endpoint
            ...
            clean_values = _compute_scalar_payloads(
                ...
                batch_pair_ids=batch_pair_ids,
                request_specs_by_pair_id=request_specs_by_pair_id,
                label="residual-position rescue clean",
            )
            ablated_values = _compute_scalar_payloads(
                answer_logits=ablated_logits,
                answer_targets=ablated_targets,
                ...
                batch_pair_ids=batch_pair_ids,
                request_specs_by_pair_id=request_specs_by_pair_id,
                label="residual-position rescue ablated",
            )
            patched_values_by_stage_role: dict[tuple[str, str], dict[tuple[int, int, str, str, str, str], float]] = {}
            for patch_stage in patch_stages:
                clean_stage = clean_outputs.residual_streams[patch_stage].detach()
                ablated_stage = ablated_outputs.residual_streams[patch_stage].detach()
                for position_role in position_roles:
                    patch = _position_limited_residual_patch(
                        clean_residual=clean_stage,
                        ablated_residual=ablated_stage,
                        batch=batch,
                        metadata=clean_metadata,
                        position_role=position_role,
                    )
                    with torch.no_grad():
                        patched_outputs = model(
                            batch["input_ids"],
                            attention_mask=batch["attention_mask"],
                            residual_patch={patch_stage: patch},
                            **source_mask_kwargs,
                        )
                    patched_logits, patched_targets, patched_metadata = extract_answer_logits(
                    ...
                    _validate_single_query_batch(
                        batch=batch,
                        metadata=patched_metadata,
                        label=f"residual-position rescue patched {patch_stage}/{position_role}",
                    )
                    patched_values_by_stage_role[(patch_stage, position_role)] = _compute_scalar_payloads(
                        answer_logits=patched_logits,
                        answer_targets=patched_targets,
                        metadata=patched_metadata,
                        batch_pair_ids=batch_pair_ids,
            ...
            for request_id, clean_scalar in clean_values.items():
                ...
                for patch_stage in patch_stages:
                    for position_role in position_roles:
                        patched_values = patched_values_by_stage_role[(patch_stage, position_role)]
                        if request_id not in patched_values:
                            raise KeyError(
                                f"Missing patched scalar for request {request_id} "
                                f"stage={patch_stage} role={position_role}."
                            )
                        patched_scalar = patched_values[request_id]
                        rescue = patched_scalar - ablated_scalar
                        unrecovered = clean_scalar - patched_scalar
    ...
    if not rows:
        raise RuntimeError(f"No residual-position rescue rows produced for source component {source_component}.")
    return rows
...
def summarize_position_rescue_rows(*, rows: list[dict[str, Any]], denominator_threshold: float) -> list[dict[str, Any]]:
    if not rows:
        raise ValueError("Cannot summarize empty residual-position rescue rows.")
    if denominator_threshold < 0.0:
        raise ValueError("denominator_threshold must be non-negative.")
    summaries: list[dict[str, Any]] = []
    group_keys = sorted(
    ...
    for scalar_name, endpoint_kind, source_component, patch_stage, position_role in group_keys:
        group = [
            row
            for row in rows
            if str(row["scalar_name"]) == scalar_name
            ...
            and str(row["source_component"]) == source_component
            and str(row["patch_stage"]) == patch_stage
            and str(row["position_role"]) == position_role
        ]
        total_drops = [float(row["total_drop"]) for row in group]
        rescues = [float(row["rescue"]) for row in group]
        unrecovered = [float(row["unrecovered"]) for row in group]
        ...
        mean_total_drop = _mean(
            total_drops,
            label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/drop",
        )
        mean_rescue = _mean(
            rescues,
            label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/rescue",
        )
        summaries.append(
            {
                "scalar_name": scalar_name,
                ...
                "mean_clean_scalar": _mean(
                    [float(row["clean_scalar"]) for row in group],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/clean",
                ),
                "mean_source_ablated_scalar": _mean(
                    [float(row["source_ablated_scalar"]) for row in group],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/ablated",
                ),
                "mean_patched_scalar": _mean(
                    [float(row["patched_scalar"]) for row in group],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/patched",
                ),
                "mean_total_drop": mean_total_drop,
                "mean_abs_total_drop": _mean(
                    [abs(value) for value in total_drops],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/abs drop",
                ),
                "mean_rescue": mean_rescue,
                "mean_abs_rescue": _mean(
                    [abs(value) for value in rescues],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/abs rescue",
                ),
                "mean_unrecovered": _mean(
                    unrecovered,
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/unrecovered",
                ),
                "mean_abs_unrecovered": _mean(
                    [abs(value) for value in unrecovered],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/abs unrecovered",
                ),
                "mean_rescue_fraction_from_means": None
                if abs(mean_total_drop) <= denominator_threshold
                else float(mean_rescue / mean_total_drop),
                ...
                if not recovery_fraction_rows
                    ...
                    recovery_fraction_rows,
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/fraction rows",
                ),
                "improved_fraction": _mean(
                    [1.0 if bool(row["improved_by_patch"]) else 0.0 for row in group],
                    label=f"{scalar_name}/{endpoint_kind}/{source_component}/{patch_stage}/{position_role}/improved",
                ),
                "rescue_vs_drop_r_squared": _safe_r_squared(
                    y_values=total_drops,
                    predicted_values=rescues,
...
def _plot_position_rescue_fraction(
    *,
    summary_rows: list[dict[str, Any]],
    output_path: Path,
    top_k_rows: int,
    ...
    fig, ax = plt.subplots(figsize=(max(10, 0.75 * len(rows)), 6))
    labels = [
        f"{row['scalar_name']}:{row['source_component']}:{row['patch_stage']}:{row['position_role']}"
        for row in rows
    ]
    values = [
        0.0 if row["mean_rescue_fraction_from_means"] is None else float(row["mean_rescue_fraction_from_means"])
    ...
    ax.axhline(1.0, color="#777777", linewidth=1.0, linestyle="--")
    ax.set_ylabel("mean rescue / mean source-ablation drop")
    ax.set_title("Residual-position rescue fraction at target endpoints")
    ax.tick_params(axis="x", rotation=50)
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def _plot_position_unrecovered(
    *,
    summary_rows: list[dict[str, Any]],
    output_path: Path,
    top_k_rows: int,
    ...
    ax.axhline(0.0, color="#777777", linewidth=1.0)
    ax.set_ylabel("mean abs clean - patched scalar")
    ax.set_title("Residual-position rescue unrecovered effect at target endpoints")
    ax.tick_params(axis="x", rotation=50)
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def _write_position_markdown(
    *,
    ...
    lines = [
        "# Residual-Position Rescue",
        "",
        "## Calculation",
        "",
        "This report tests whether clean residual vectors at selected token positions can rescue the model after a source component is ablated.",
        "",
        "```text",
        "damage = scalar(clean) - scalar(source ablated)",
        "patch = ablated residual state, with clean vectors copied only at position_role positions",
        "rescue = scalar(source ablated + position-limited patch) - scalar(source ablated)",
        "unrecovered = scalar(clean) - scalar(source ablated + position-limited patch)",
        "rescue_fraction = rescue / damage",
        "```",
        "",
        "A high rescue fraction means the missing causal information is carried by the selected positions at that residual stage. A low rescue fraction means that position role alone does not restore the source component's effect.",
        "",
        "## Inputs",
        "",
        f"- scalar pair rows: `{report['scalar_pair_rows_path']}`",
        ...
        f"- source components: `{report['source_components']}`",
        f"- patch stages: `{report['patch_stages']}`",
        f"- position roles: `{report['position_roles']}`",
        "",
        "## Summary",
        "",
        "| scalar | endpoint | source | patch stage | position role | observations | damage | rescue | rescue fraction | abs unrecovered | improved fraction | corr | R squared |",
        "| --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in sorted(
        report["summary_rows"],
        ...
        r2 = row["rescue_vs_drop_r_squared"]
        lines.append(
            "| {scalar} | {endpoint} | `{source}` | `{stage}` | `{role}` | {n} | {damage:.6g} | {rescue:.6g} | {fraction} | {unrecovered:.6g} | {improved:.3f} | {corr} | {r2} |".format(
                scalar=row["scalar_name"],
                endpoint=row["endpoint_kind"],
                source=row["source_component"],
                stage=row["patch_stage"],
...
def run_residual_position_rescue(
    *,
    config_path: Path,
    probe_set_path: Path,
    scalar_pair_rows_path: Path,
    ...
    plot_top_k_rows: int = 64,
    overwrite: bool = False,
) -> tuple[Path, Path, Path, Path, Path, dict[str, Path]]:
    if not pair_types:
        raise ValueError("residual-position-rescue requires at least one pair type.")
    if not source_components:
        raise ValueError("source_components must not be empty.")
    if not patch_stages:
        raise ValueError("patch_stages must not be empty.")
    if not position_roles:
        raise ValueError("position_roles must not be empty.")
    if max_pairs_per_type <= 0:
        raise ValueError("max_pairs_per_type must be positive.")
    if min_pairs_per_type <= 0:
        raise ValueError("min_pairs_per_type must be positive.")
    ...
    print(
        "[residual-position-rescue] "
        f"pairs={len(pairs_by_id)} scalar_rows={len(scalar_pair_rows)} endpoints={len(endpoint_keys)} "
        f"sources={resolved_sources} patch_stages={resolved_patch_stages} position_roles={resolved_position_roles} "
    ...
    for source_component in resolved_sources:
        print(
            f"[residual-position-rescue] source={source_component} running clean/ablated/position-patched forwards",
            flush=True,
        )
        rescue_rows.extend(
            _compute_position_rescue_rows_for_source(
                model=model,
                checkpoint_paths_by_step=checkpoint_paths_by_step,
                pairs_by_id=pairs_by_id,
                endpoint_keys=endpoint_keys,
    ...
    summary_rows = summarize_position_rescue_rows(rows=rescue_rows, denominator_threshold=denominator_threshold)

    rescue_rows_path = output_dir / "residual_position_rescue_rows.jsonl"
    summary_rows_path = output_dir / "residual_position_rescue_summary_rows.jsonl"
    pair_rows_path = output_dir / "residual_position_rescue_pairs.jsonl"
    report_path = output_dir / "residual_position_rescue_report.json"
    markdown_path = output_dir / "residual_position_rescue_report.md"
    write_jsonl(rescue_rows_path, rescue_rows)
    write_jsonl(summary_rows_path, summary_rows)
    write_jsonl(
        pair_rows_path,
    ...
    plot_paths: dict[str, Path] = {}
    fraction_plot = _plot_position_rescue_fraction(
        summary_rows=summary_rows,
        output_path=output_dir / "residual_position_rescue_fraction.svg",
        top_k_rows=plot_top_k_rows,
    )
    if fraction_plot is not None:
        plot_paths["rescue_fraction"] = fraction_plot
    unrecovered_plot = _plot_position_unrecovered(
        summary_rows=summary_rows,
        output_path=output_dir / "residual_position_rescue_unrecovered.svg",
        top_k_rows=plot_top_k_rows,
    )
    if unrecovered_plot is not None:
        plot_paths["unrecovered"] = unrecovered_plot
    ...
    report = {
        "schema_version": RESIDUAL_POSITION_RESCUE_SCHEMA_VERSION,
        "config_path": str(config_path),
        "probe_set_path": str(probe_set_path),
        "scalar_pair_rows_path": str(scalar_pair_rows_path),
        "device": device_name,
        ...
        "calculation": {
            "damage": "scalar(clean) - scalar(source ablated)",
            "rescue": "scalar(source ablated + clean residual vectors at selected positions) - scalar(source ablated)",
            "unrecovered": "scalar(clean) - scalar(source ablated + clean residual vectors at selected positions)",
            "rescue_fraction": "rescue / damage where |damage| > denominator_threshold",
            "patch_scope": "ablated residual stream tensor with clean vectors copied only at position_role positions",
        },
        "rescue_rows_path": str(rescue_rows_path),
        "summary_rows_path": str(summary_rows_path),
        "pair_rows_path": str(pair_rows_path),
    ...
    write_json(report_path, report)
    _write_position_markdown(path=markdown_path, report=report, plot_paths=plot_paths)
    print(f"[residual-position-rescue] complete report={report_path} rows={rescue_rows_path}", flush=True)
    return report_path, markdown_path, rescue_rows_path, summary_rows_path, pair_rows_path, plot_paths
